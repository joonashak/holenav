...
const addEmptyConnection = (signature: CreatableSignature): CreatableSignature => ({
  wormholeType: "",
  ...signature,
  connection: {
    eol: false,
...
export const completeSignature = (
  ...
  const whWithConnection = signature.connection ? signature : addEmptyConnection(signature);
